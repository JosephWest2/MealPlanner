...
export default function RecipeSearch() {
    ...
    return (<div className="box column">
        <div className="row">
            <input type="text" placeholder="Search..." style={{ width: "100%" }} onKeyDown={SubmitOnEnter} onChange={e => setQuery(e.target.value)} value={query} />
        ...
        <div className="column toggleable" data-enabled={showMoreOptions.toString()}>
            ...
            <div className="row">
                ...
                <div className="row" style={{ gap: "0", height: "2rem" }}>
            ...
            <ul>
                {Object.entries(nutrientLimits).map(([key, value]) => (
                    <li className="row" key={key}>{key}: {value}<button style={{ padding: "0.5rem", height: "2rem", width: "2rem", textAlign: "center" }} onClick={() => RemoveNutrientLimit(key)}>X</button></li>
                ))}
            </ul>
        </div>
